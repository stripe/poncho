...
class TestValidation < Test
  ...
  def test_format_options
    ...
    # Valid
    Poncho::Validations::FormatValidator.new(valid_args.dup)

    Poncho::Validations::FormatValidator.new(
    ...
    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.reject{|k, v| k == :attributes})
    end

    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.merge(:attributes => []))
    end

    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.merge(:attributes => 'baz'))
    end

    # Invalid other options
    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.reject{|k, v| k == :with})
    end

    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.merge(:without => /baz/))
    end

    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.merge(:with => '/baz/'))
    end

    assert_raises ArgumentError do
      Poncho::Validations::FormatValidator.new(valid_args.reject{|k, v| k == :message})
    end
  end
  ...
  def test_format_validation
    ...
    with_validator = Poncho::Validations::FormatValidator.new(
    ...
    without_validator = Poncho::Validations::FormatValidator.new(
  ...
  def test_exclusions_validation
    ...
    array_validator = Poncho::Validations::ExclusionValidator.new(
      :in => [1, "a", false], :attributes => [:foo])
    lambda_validator = Poncho::Validations::ExclusionValidator.new(
      :in => lambda{|x| [2, 'b', true]}, :attributes => [:foo])
    range_validator = Poncho::Validations::ExclusionValidator.new(
  ...
  def test_inclusions_validation
    ...
    array_validator = Poncho::Validations::InclusionValidator.new(
      :in => [1, "a", false], :attributes => [:foo])
    lambda_validator = Poncho::Validations::InclusionValidator.new(
      :in => lambda{|x| [2, 'b', true]}, :attributes => [:foo])
    range_validator = Poncho::Validations::InclusionValidator.new(
      :in => 0..10, :attributes => [:foo])
